...
CREATE TABLE IF NOT EXISTS `enjoytrip`.`board` (
  ...
  `board_intro` VARCHAR(100) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `enjoytrip`.`article` (
  ...
  PRIMARY KEY (`article_no`, `board_type`),
  INDEX `board_article_board_type_idx` (`board_type` ASC) VISIBLE,
  INDEX `user_article_user_id_idx` (`user_id` ASC) VISIBLE,
  CONSTRAINT `board_article_board_type`
    FOREIGN KEY (`board_type`)
    REFERENCES `enjoytrip`.`board` (`board_type`),
  CONSTRAINT `user_article_user_id`
    FOREIGN KEY (`user_id`)
...
CREATE TABLE IF NOT EXISTS `enjoytrip`.`comment` (
  ...
  INDEX `board_comment_board_type_idx` (`board_type` ASC) VISIBLE,
  INDEX `article_to_comment_article_no_idx` (`article_no` ASC) VISIBLE,
  INDEX `user_to_comment_user_id_idx` (`user_id` ASC) VISIBLE,
  CONSTRAINT `article_to_comment_article_no`
    FOREIGN KEY (`article_no`)
    REFERENCES `enjoytrip`.`article` (`article_no`),
  CONSTRAINT `board_to_comment_board_type`
    FOREIGN KEY (`board_type`)
    REFERENCES `enjoytrip`.`board` (`board_type`),
  CONSTRAINT `user_to_comment_user_id`
    FOREIGN KEY (`user_id`)
    REFERENCES `enjoytrip`.`user` (`id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
...
-- -----------------------------------------------------
-- Table `enjoytrip`.`myplan_board`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `enjoytrip`.`myplan_board` (
  `article_no` INT NOT NULL AUTO_INCREMENT,
  `user_id` VARCHAR(40) NOT NULL,
...
-- -----------------------------------------------------
-- Table `enjoytrip`.`myplan_attraction`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `enjoytrip`.`myplan_attraction` (
  `myplan_no` INT NOT NULL,
  `content_id` INT NOT NULL,
  `content_order` INT NOT NULL,
  INDEX `attraction_content_id_idx` (`content_id` ASC) VISIBLE,
  INDEX `pk_myplanno_userid_idx` (`myplan_no` ASC) VISIBLE,
  CONSTRAINT `attraction_content_id`
    FOREIGN KEY (`content_id`)
    REFERENCES `enjoytrip`.`attraction_info` (`content_id`),
  CONSTRAINT `pk_myplanno_userid`
    FOREIGN KEY (`myplan_no`)
    REFERENCES `enjoytrip`.`myplan_board` (`article_no`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;
