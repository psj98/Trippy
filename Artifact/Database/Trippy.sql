...
CREATE TABLE IF NOT EXISTS `enjoytrip`.`comment` (
  ...
  INDEX `article_to_comment_board_type_idx` (`board_type` ASC) VISIBLE,
  CONSTRAINT `article_to_comment_article_no`
    FOREIGN KEY (`article_no`)
    REFERENCES `enjoytrip`.`article` (`article_no`),
  CONSTRAINT `article_to_comment_board_type`
    FOREIGN KEY (`board_type`)
    REFERENCES `enjoytrip`.`article` (`board_type`),
...
CREATE TABLE IF NOT EXISTS `enjoytrip`.`hotplace` (
  ...
  CONSTRAINT `attraction_info_to_attr_planned_content_id`
    FOREIGN KEY (`content_id`)
    REFERENCES `enjoytrip`.`attraction_info` (`content_id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
...
CREATE TABLE IF NOT EXISTS `enjoytrip`.`myplan_board` (
  ...
  `attraction_id_list` VARCHAR(100) NOT NULL,
  `hit` INT NULL DEFAULT '0',
  `register_time` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `share_check` INT NULL DEFAULT '0',
